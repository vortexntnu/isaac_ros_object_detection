...
class Yolov26Visualizer(Node):
    QUEUE_SIZE = 10
    color = (0, 255, 0)
    bbox_thickness = 2

    def __init__(self):
        super().__init__('yolov26_visualizer')
        self._bridge = cv_bridge.CvBridge()
        self._processed_image_pub = self.create_publisher(
            Image, 'yolov26_processed_image', self.QUEUE_SIZE)
    ...
    def _draw_rotated(self, img, cx, cy, w, h, theta_rad):
        rect = ((float(cx), float(cy)), (float(w), float(h)), float(theta_rad * 180.0 / math.pi))
        box = cv2.boxPoints(rect)
        box = box.astype(int)
        cv2.polylines(img, [box], isClosed=True, color=self.color, thickness=self.bbox_thickness)
        return box
    ...
    def detections_callback(self, detections_msg, img_msg):
        ...
        for detection in detections_msg.detections:
            ...
            if abs(theta) > 1e-6:
                box = self._draw_rotated(cv2_img, center_x, center_y, width, height, theta)
                ...
                cv2.putText(
                    cv2_img, label,
                    (min_pt[0], min_pt[1]-2 if min_pt[1] - 10 >= 3 else min_pt[1] + 10),
